...
FIRST_APPENDIX = LITERATURE / "FOUNDER_CITATION_PASS_0003_APPENDIX_v1.0.0.csv"
FIRST_PACKET = LITERATURE / "FOUNDER_CITATION_PASS_0003_PACKET_v1.0.0.md"
SECOND_APPENDIX = (
    LITERATURE / "FOUNDER_CITATION_PASS_0003_ADJUDICATION_APPENDIX_v1.0.0.csv"
)
SECOND_PACKET = (
    LITERATURE / "FOUNDER_CITATION_PASS_0003_ADJUDICATION_PACKET_v1.0.0.md"
)
COMBINED_REVIEW = LITERATURE / "FOUNDER_CITATION_PASS_0003_COMBINED_REVIEW_v1.0.0.md"
FIRST_RECEIPT = LITERATURE / "citation-chain" / "pass-0003-founder-packet.yaml"
SECOND_RECEIPT = (
    LITERATURE / "citation-chain" / "pass-0003-adjudication-packet.yaml"
)
...
def test_pass3_combined_packet_covers_every_candidate_once() -> None:
    first = _rows(FIRST_APPENDIX)
    second = _rows(SECOND_APPENDIX)
    first_decided = [row for row in first if row["recommendation"] != "unclear"]

    assert len(first) == 593
    assert len(first_decided) == 564
    assert len(second) == 29
    combined = first_decided + second
    assert len(combined) == 593
    assert len({row["record_key"] for row in combined}) == 593
    assert sum(row["recommendation"] == "include" for row in combined) == 7
    assert sum(row["recommendation"] == "exclude" for row in combined) == 586
...
def test_pass3_review_binds_both_verified_packet_pairs() -> None:
    ...
    assert "# Founder Citation Screening Packet — Pass 3" in FIRST_PACKET.read_text()
    assert "# Founder Citation Screening Packet — Pass 3" in SECOND_PACKET.read_text()
    assert "Unique candidate records: 593" in text
    assert "Proposed includes: 7" in text
    assert "Proposed excludes: 586" in text
    assert (
        "`I confirm both checksum-bound citation pass 3 packets as written.`"
        in text
    )


def test_pass3_confirmation_path_is_ready_without_recording_a_decision() -> None:
    ...
    receipt = CitationDecisionConfirmationService(store=store).confirm(
        ...
        code_revision="6329453",
    )

    assert receipt.pass_number == 3
    assert receipt.candidate_count == 593
    assert receipt.included_count == 7
    assert receipt.excluded_count == 586
